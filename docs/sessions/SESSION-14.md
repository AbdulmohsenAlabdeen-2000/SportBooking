...
- **Webhook deliveries not arriving at `/api/payments/webhook`.** No POSTs visible in Vercel logs after payment attempts. Most likely cause: `MYFATOORAH_WEBHOOK_SECRET` on Vercel doesn't match the secret saved on the MyFatoorah Webhook Settings page (the Generate New Key flow may have rotated it). Action: re-copy from the portal, `vercel env rm` + `vercel env add`, redeploy. Not blocking the customer flow — page handles both terminal states — but needed for refund-status events and as a safety net.
